...
class TestRecursiveRemove(unittest.TestCase):
    ...
    def test_repository_only(self):
        ...
        self.client.post(CONTAINER_RECURSIVE_ADD_PATH, {'repository': self.to_repo['_href']})
        after_add_version_href = self.client.get(self.to_repo['_href'])['_latest_version_href']

        # Actual test
        self.client.post(CONTAINER_RECURSIVE_REMOVE_PATH, {'repository': self.to_repo['_href']})
